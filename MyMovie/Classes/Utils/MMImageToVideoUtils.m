...
+(void)videoFromImageModel:(MMMediaImageModel *)model onQueue:(dispatch_queue_t)queue docPath:(NSString*)docPath complete:(void (^)(NSString *))complete {
    ...
    AVAssetWriter* assetWriter = [AVAssetWriter assetWriterWithURL:fileURL fileType:AVFileTypeMPEG4 error:nil];
    
    AVAssetWriterInput* assetWriterInput = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeVideo outputSettings:@{AVVideoCodecKey : AVVideoCodecH264, AVVideoWidthKey : [NSNumber numberWithDouble:k1280pSize.width], AVVideoHeightKey : [NSNumber numberWithDouble:k1280pSize.height]}];
    assetWriterInput.expectsMediaDataInRealTime = NO;
    
    AVAssetWriterInputPixelBufferAdaptor* adaptor = [AVAssetWriterInputPixelBufferAdaptor assetWriterInputPixelBufferAdaptorWithAssetWriterInput:assetWriterInput sourcePixelBufferAttributes:attrDict];
...
+(void)transitionInstructionsInVideoComposition:(AVVideoComposition*)videoComposition videoAssets:(NSArray*)videoAssets {
    ...
    for (AVMutableVideoCompositionInstruction* vci in videoComposition.instructions) {
        if(vci.layerInstructions.count == 2) {
            ...
            AVMutableVideoCompositionLayerInstruction* fromLayerInstruction = (AVMutableVideoCompositionLayerInstruction *)[vci.layerInstructions objectAtIndex:1 - layerInstructionIndex];
            AVMutableVideoCompositionLayerInstruction* toLayerInstruction = (AVMutableVideoCompositionLayerInstruction *)[vci.layerInstructions objectAtIndex:layerInstructionIndex];
